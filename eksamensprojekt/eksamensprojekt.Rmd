...
```{r get-document}
german_path <- here("data", "TroveApiData100prDecade.csv")
german_df <- read_csv("data/TroveApiData100prDecade.csv")
...
There are 159,939 "words" (1 or more letter(s) with spaces or punctuation on either side) in the articles altogether.
...
Excludes germany and german which is good, because they shouldn't have a sentimental value. The word "ger" is probably showing us that there are 259 instances where "german\*" is separated by a new line.
...
Again, only one word was analyzed by AFINN in this article, thereby the very positive score.

This makes me wonder how many words that are actually recognized by AFINN on average in the articles, which could provide a significant source of error. This will be investigated in the analysis.
...
### After WW2 (1945/05/07 -- 1999/12/13)
...
### Sentiment analysis after WW2 1945 - 1999:

```{r}
german_afinn_45_99 <- german_afinn %>% 
  filter(between(date, as.Date("1945-05-07"), as.Date("1999-12-31")))

german_afinn_45_99_summary <- german_afinn_45_99 %>% 
  summarize(
    time_period = "1945-1999",
    mean_score = mean(value),
    median_score = median(value),
    AFINN_word_count_per_article = nrow(german_afinn_45_99) / nrow(german_df %>% filter(between(date, as.Date("1945-05-07"), as.Date("1999-12-31"))))
  )
german_afinn_45_99_summary
```
...
german_afinn_vertsum <- german_afinn_00_14_summary %>% 
  bind_rows(german_afinn_14_18_summary, german_afinn_19_33_summary, german_afinn_33_39_summary, german_afinn_39_45_summary, german_afinn_45_99_summary)
...
```{r}
german_nrc <- german_stop %>% 
  inner_join(get_sentiments("nrc"))
```

Check exclusions, count which is most excluded:

```{r}
german_exclude_nrc <- german_stop %>% 
  anti_join(get_sentiments("nrc"))
#view(german_exclude_nrc)

german_exclude_nrc_n <- german_exclude_nrc %>% 
  count(word, sort = TRUE)

head(german_exclude_nrc_n)
```

ok, nrc excludes "german" and "germany"

## Finding some counts

```{r}
german_nrc_n <- german_nrc %>% 
  count(sentiment, sort = TRUE)

# And plot them:

ggplot(data = german_nrc_n, aes(x = sentiment, y = n)) +
  geom_col()
```
